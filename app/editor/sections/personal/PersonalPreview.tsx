...
export function PersonalPreview({ data, socialLinks = [], viewMode, previewMode }: PersonalPreviewProps) {
  ...
  return (
    <div id="overview" className={`w-full bg-white ${isMobile ? 'px-4 py-8' : 'px-4 sm:px-6 lg:px-8'}`}>
      <div className={`max-w-6xl mx-auto flex ${isMobile ? 'flex-col items-center text-center gap-6' : 'items-start gap-8 sm:gap-12'}`}>
        ...
        <div className={`flex-1 ${isMobile ? 'w-full' : ''}`}>
          ...
          {socialLinks.length > 0 && (
            <div className={`flex flex-wrap gap-2 mb-4 sm:mb-6 ${isMobile ? 'justify-center' : ''}`}>
          ...
          {/* About (Below social links) */}
          {data.whoAreYou && (
            <p className={`text-gray-600 leading-relaxed mb-12 ${isMobile ? 'text-sm' : 'text-base'}`}>
              {data.whoAreYou}
            </p>
          )}
        </div>
